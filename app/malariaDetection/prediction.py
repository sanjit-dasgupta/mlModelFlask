...
class MalariaDetection():
    def __init__(self,model_file='rf_ensemblev3.sav'):
    ...
    def load_image(self,imgFileStorage):
        ...
        #self.im = cv2.imread(self.img_path, cv2.IMREAD_COLOR)
        self.im = cv2.GaussianBlur(self.img,(5,5),2)
        self.im_gray = cv2.cvtColor(self.im,cv2.COLOR_BGR2GRAY)
        ret, self.thresh = cv2.threshold(self.im_gray,127,255,0)
    ...
    def predict(self, imgFileStorage):
        ...
        result = "Plasmodium Detected" if self.predict_outcome()[0] == "Parasitized" else "Plasmodium Not Detected"
        original_image, plasmodium_image = self.show_plasmodium()
        return (result, original_image, plasmodium_image)